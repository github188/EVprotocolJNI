...
volatile unsigned char pcLock;//PCÇëÇó»¥³âËø
volatile unsigned char pcType,pcsubType;//PCÇëÇóÀàÐÍ 0±íÊ¾ÎÞÇëÇó±êÖ¾¿ÕÏÐ
volatile unsigned char pcFlag;//VMC 0¿ÕÏÐ 1ÐèÒª·¢ËÍÇëÇó  2ÕýÔÚ´¦ÀíÇëÇó
...
{	
	...
	if(snNo == recvbuf[SN])//ÖØ°ü Ö±½ÓÅ×Æú
    {
...
int EV_vmMainFlow(const unsigned char type,const unsigned char *data,const unsigned char len)
{
	...
	{
		case EV_NAK_VM:
			if(EV_get_pc_cmd() != EV_NA)//ÓÐÃüÁî±»¾Ü¾ø
			{
				EV_set_pc_cmd(EV_NA);
				EV_LOGFLOW("EV_NA\n");
			...
			break;
        case EV_SETUP_REQ:	//	1 ³õÊ¼»¯ GET_SETUP PC·¢ËÍ³õÊ¼»¯ÃüÁî
		...
		case EV_SETUP_RPT://³õÊ¼»¯·µ»ØÐÅÏ¢
			...
			EV_set_pc_cmd(EV_NA);
			if(EV_getVmState() != EV_STATE_INITTING) //ÅÐ¶ÏÊÇ·ñÔÚ³õÊ¼»¯
			{
				break;
			}//ÊÇ³õÊ¼»¯ Ö±½ÓÏÂÒ»²½	
		
		case EV_CONTROL_REQ://·¢ËÍ³õÊ¼»¯Íê³É±êÖ¾
		...
		case EV_CONTROL_RPT:
			if(EV_get_pc_cmd() == EV_CONTROL_REQ)
				EV_set_pc_cmd(EV_NA);
			if(EV_getSubcmd() == 19)//³õÊ¼»¯Íê³É±êÖ¾ 
			{
				if(EV_getVmState() != EV_STATE_INITTING)//ÊÇ×Ô¶¯³õÊ¼»¯Ìø¹ýÖ±½ÓÏÂÒ»²½
				{
					break;
				}
			}
		...
		case EV_STATE_RPT://×´Ì¬·µ»Ø
			...
			if(EV_getVmState() == EV_STATE_INITTING)//³õÊ¼»¯×´Ì¬ ±íÊ¾³õÊ¼»¯Íê±Ï
			{
			...
			else if(EV_getVmState() == EV_STATE_MANTAIN)//×´Ì¬ÊÇÎ¬»¤ ²»¸ü¸Ä
			{
				temp = EV_getVmState();
		...
		case EV_OFFLINE://ÀëÏß
			...
			break;
		case EV_ACTION_REQ: // PC¶¯×÷ÇëÇó Ä¿Ç°²»ÓÃ
			
			break;
		case EV_ACTION_RPT: //VMC¶¯×÷±¨¸æ 
			if(data[MT + 1] == 7)//VMCÖØÆô±¨¸æ
			{
			...
			break;
		case EV_POLL:
			if(EV_getVmState() == EV_STATE_MANTAIN)//±íÊ¾ÍË³öÎ¬»¤Ä£Ê½
			{
				EV_setVmState(EV_STATE_INITTING);
...
int32_t	EV_vmRpt(const uint8_t type,const uint8_t *data,const uint8_t len)
{
	...
	if(type == EV_POLL)
	{
		if(EV_getVmState() == EV_STATE_DISCONNECT)//Èç¹û¶ÏÏßÁËÔò×Ô¶¯½øÈë³õÊ¼»¯Á÷³Ì
		{
